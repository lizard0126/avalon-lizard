...
function apply(ctx, config) {
  ...
  avalonCommand.subcommand(".规则", "查看游戏规则").action(async ({ session }) => {
    const rules = `
阿瓦隆游戏规则
...
你可以投任务成功，也可以投任务失败。`;
      ...
      await session.bot.sendPrivateMessage(player.id, `${import_koishi.h.image(imageUrl)}
${message}`);
    }
    game.round = 1;
...
当前发车人是：${game.players[game.currentLeaderIndex].name}，请在当前群聊使用指令“阿瓦隆 发车 玩家1,玩家2”进行选择
  ...
  avalonCommand.subcommand(".发车 <players:string>", "选择任务成员").action(async ({ session }, players) => {
    ...
    if (game.round > 0 && game.currentVotes) {
      const allVoted = Object.values(game.currentVotes).every((vote) => vote !== void 0);
      if (!allVoted) {
    ...
    const selectedIds = selectedNames.map((name2) => {
      const player = game.players.find((p) => p.name.toLowerCase() === name2.toLowerCase());
